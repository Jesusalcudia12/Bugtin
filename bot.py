...
def verificar_url(url):
    """Verifica si un subdominio responde y obtiene su IP"""
    ...
    try:
        ...
        if response.status_code < 400:
            return f"✅ {url} [{ip}] (Status: {response.status_code})"
        return f"❌ {url} [{ip}] (Status: {response.status_code})"
    except:
        try:
            ip = socket.gethostbyname(hostname)
            return f"❌ {url} [{ip}] (No responde)"
        except:
...
def do_dir(message):
    ...
    bot.send_message(message.chat.id, "🚀 Escaneando archivos sensibles y rutas...")
    # Buscando activamente en maestra.txt y extensiones críticas
    cmd = (
...
def do_subs(message):
    ...
    bot.send_message(message.chat.id, f"📡 Extrayendo y validando subdominios con IP para `{dom}`...")
    ...
    if os.path.exists(path_raw):
        ...
        bot.send_message(message.chat.id, "❌ No se encontraron subdominios.")

@bot.message_handler(commands=['fuerza'])
def cmd_fuerza(message):
    bot.send_message(message.chat.id, "⚡ **Formato: `IP Servicio`**\nEl bot usará `maestra.txt` como lista de usuarios.")
...
def do_fuerza(message):
    try:
        ...
        wordlist = preparar_wordlist() # Usa maestra.txt
        bot.send_message(message.chat.id, f"⚡ Iniciando Hydra contra {service} en {target}...")
        # L = maestra.txt para usuarios, P = maestra.txt para passwords
...
def do_audit(message):
    ...
    # -rl: rate limit para no ser bloqueado | -as: automatic scan
...
if __name__ == "__main__":
    print("🚀 Bugtin Bot v22.0 Online by Owen")
