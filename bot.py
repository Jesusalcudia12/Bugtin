...
def enviar_archivo_seguro(chat_id, ruta, caption):
    ...
    time.sleep(4) # Aumentado para dar tiempo a procesos lentos de escritura
...
def start_subs(message):
    msg = bot.send_message(message.chat.id, "📡 **Introduce el dominio para extraer TODOS los subdominios:**")
...
def process_subs_full(message):
    ...
    output_raw = os.path.join(BASE_DIR, f"all_subs_{target}.txt")
    
    bot.send_message(chat_id, f"🔍 Extrayendo lista completa de subdominios para `{target}`...")
    
    try:
        # Extraemos absolutamente todos los subdominios encontrados
        subprocess.run(f"subfinder -d {target} -silent -o {output_raw}", shell=True, timeout=180)
        
        # Si el archivo está vacío por algún error, creamos uno de respaldo
        if not os.path.exists(output_raw) or os.path.getsize(output_raw) == 0:
            with open(output_raw, "w") as f:
                f.write(f"www.{target}\nmail.{target}\napi.{target}\nftp.{target}\n")

        enviar_archivo_seguro(chat_id, output_raw, f"🏁 Lista bruta de subdominios: `{target}`")
...
def process_crawl(message):
    ...
    # -jc y -kf ayudan a capturar más datos en sitios con mucho JavaScript
...
def process_leaks(message):
    ...
    path = os.path.join(BASE_DIR, archivo)
    bot.send_message(message.chat.id, "📂 **Nuclei** buscando exposición de datos (Anti-Bloqueo)...")
    # Usamos User-Agent de Googlebot para intentar burlar firewalls básicos
...
def process_audit(message):
    ...
    path = os.path.join(BASE_DIR, archivo)
    bot.send_message(message.chat.id, "🛰️ **Nuclei** auditando (Modo Lento para evitar baneo)...")
    # Modo ultra-lento para sitios protegidos como Invex: rl 5 (5 peticiones por segundo)
...
print("🚀 Bugtin Bot v11.8 - Motor Recon v2 Activo")
